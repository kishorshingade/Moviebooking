...
const Navbar = () => {
  ...
  return (
    ...
    justify-between px-6 md:px-16 lg:px-36 py-5 '>
      ...
      <div className={`
  z-50 flex flex-col md:flex-row items-center gap-8 py-3 transition-[width] duration-300
  backdrop:blur bg-black/74 md:bg-white/10 md:border border-gray-300/20 overflow-hidden
  ${isOpen ? 'max-md:max-w-full max-md:w-full' : 'max-md:w-0'}
  max-md:absolute max-md:top-0 max-md:left-0 max-md:font-medium max-md:text-lg max-md:justify-center max-md:h-screen
  md:rounded-full md:px-8
`}>
        ...
        <SearchIcon className='max-md:hidden w-6 h-6 cursor-pointer' />
          {
            !user ? (
              ...
              hover:bg-primary-dull trasnsition rounded-full font-medium cursor-pointer
              '>Login</button>
            ) : (
              ...
              <UserButton>
                <UserButton.MenuItems>
                  ...
                  onClick={()=> useNavigate('/my-bookings')} />
                </UserButton.MenuItems>
              </UserButton>
